...
function wp_allow_comment($commentdata) {
	...
	$dupe = $wpdb->prepare( "SELECT \"comment_ID\" FROM $wpdb->comments WHERE \"comment_post_ID\" = %d AND comment_parent = %s AND comment_approved != 'trash' AND ( comment_author = %s ", $wpdb->_real_escape( $comment_post_ID ), $wpdb->_real_escape( $comment_parent ), $wpdb->_real_escape( $comment_author ) );
	if ( $comment_author_email )
		$dupe .= $wpdb->prepare( "OR comment_author_email = %s ", $wpdb->_real_escape( $comment_author_email ) );
	$dupe .= $wpdb->prepare( ") AND comment_content = %s LIMIT 1", $wpdb->_real_escape( $comment_content ) );
